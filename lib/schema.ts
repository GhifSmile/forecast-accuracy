...
export const plantPerformanceDetailWeekly = pgView("plant_performance_detail_weekly", {	plant: varchar({ length: 50 }),
    ...
    w3: numeric(),
    w4: numeric(),
    mtdAvg: numeric("mtd_avg"),
    vsTarget: numeric("vs_target"),
}).as(sql`WITH code_summary AS ( SELECT t1.year, t1.month, t1.week, t1.plant, t1.business_unit, t1.code, sum(t1.forecast) AS sum_f, sum(t1.sales) AS sum_s FROM data_collection_forecast_accuracy t1 GROUP BY t1.year, t1.month, t1.week, t1.plant, t1.business_unit, t1.code ), code_error_weekly AS ( SELECT t2.year, t2.month, t2.week, t2.plant, t2.business_unit, t2.code, t2.sum_f, t2.sum_s, CASE WHEN t2.sum_f <= 0::numeric OR t2.sum_s <= 0::numeric THEN 0.0 WHEN (abs(t2.sum_f - t2.sum_s) / NULLIF(t2.sum_f, 0::numeric)) > 1::numeric THEN 0.0 ELSE abs(t2.sum_f - t2.sum_s) / NULLIF(t2.sum_f, 0::numeric) END AS valid_error_value FROM code_summary t2 ), bu_weekly_avg_error AS ( SELECT t3.year, t3.month, t3.week, t3.plant, t3.business_unit, COALESCE(sum(t3.valid_error_value) / NULLIF(count( CASE WHEN t3.valid_error_value > 0::numeric THEN 1 ELSE NULL::integer END), 0)::numeric, 0::numeric) AS average_error_rate_weekly FROM code_error_weekly t3 GROUP BY t3.year, t3.month, t3.week, t3.plant, t3.business_unit ), max_week AS ( SELECT code_summary.year, code_summary.month, code_summary.plant, code_summary.business_unit, max(code_summary.week) AS minggu_akhir FROM code_summary GROUP BY code_summary.year, code_summary.month, code_summary.plant, code_summary.business_unit ), code_accuracy_mtd AS ( SELECT cs.year, cs.month, cs.plant, cs.business_unit, cs.code, sum(cs.sum_f) AS sum_f_mtd, sum(cs.sum_s) AS sum_s_mtd FROM code_summary cs JOIN max_week mw ON cs.year = mw.year AND cs.month = mw.month AND cs.plant::text = mw.plant::text AND cs.business_unit::text = mw.business_unit::text WHERE cs.week <= mw.minggu_akhir GROUP BY cs.year, cs.month, cs.plant, cs.business_unit, cs.code ), code_error_mtd AS ( SELECT t6.year, t6.month, t6.plant, t6.business_unit, t6.code, t6.sum_f_mtd, t6.sum_s_mtd, CASE WHEN t6.sum_f_mtd <= 0::numeric OR t6.sum_s_mtd <= 0::numeric THEN 0.0 WHEN (abs(t6.sum_f_mtd - t6.sum_s_mtd) / NULLIF(t6.sum_f_mtd, 0::numeric)) > 1::numeric THEN 0.0 ELSE abs(t6.sum_f_mtd - t6.sum_s_mtd) / NULLIF(t6.sum_f_mtd, 0::numeric) END AS valid_error_value_mtd FROM code_accuracy_mtd t6 ), final_mtd_error AS ( SELECT code_error_mtd.year, code_error_mtd.month, code_error_mtd.plant, code_error_mtd.business_unit, COALESCE(sum(code_error_mtd.valid_error_value_mtd) / NULLIF(count( CASE WHEN code_error_mtd.valid_error_value_mtd > 0::numeric THEN 1 ELSE NULL::integer END), 0)::numeric, 0::numeric) AS average_error_rate_mtd FROM code_error_mtd GROUP BY code_error_mtd.year, code_error_mtd.month, code_error_mtd.plant, code_error_mtd.business_unit ), final_report AS ( SELECT t7.plant, t7.business_unit, t7.year, t7.month, max( CASE WHEN t7.week = 1 THEN round(1.0 - t7.average_error_rate_weekly, 4) ELSE NULL::numeric END) AS w1, max( CASE WHEN t7.week = 2 THEN round(1.0 - t7.average_error_rate_weekly, 4) ELSE NULL::numeric END) AS w2, max( CASE WHEN t7.week = 3 THEN round(1.0 - t7.average_error_rate_weekly, 4) ELSE NULL::numeric END) AS w3, max( CASE WHEN t7.week = 4 THEN round(1.0 - t7.average_error_rate_weekly, 4) ELSE NULL::numeric END) AS w4 FROM bu_weekly_avg_error t7 GROUP BY t7.plant, t7.business_unit, t7.year, t7.month ) SELECT t8.plant, t8.business_unit, t8.year, t8.month, t8.w1, t8.w2, t8.w3, t8.w4, round(1.0 - t9.average_error_rate_mtd, 4) AS mtd_avg, CASE WHEN t9.average_error_rate_mtd = 0::numeric THEN NULL::numeric ELSE round(1.0 - t9.average_error_rate_mtd - CASE WHEN t8.business_unit::text = 'fish'::text THEN 0.78 ELSE 0.70 END, 4) END AS vs_target FROM final_report t8 JOIN final_mtd_error t9 ON t8.year = t9.year AND t8.month = t9.month AND t8.plant::text = t9.plant::text AND t8.business_unit::text = t9.business_unit::text ORDER BY t8.year, t8.month, t8.plant, t8.business_unit`);
...
export const trendAnalysisWeekly = pgView("trend_analysis_weekly", {	year: integer(),
    month: integer(),
    week: integer(),
    overallAccuracy: numeric("overall_accuracy"),
    fishAccuracy: numeric("fish_accuracy"),
    ...
    bestPerforming: varchar("best_performing"),
    worstPerforming: varchar("worst_performing"),
}).as(sql`WITH code_weekly_metrics AS ( SELECT t1.year, t1.month, t1.week, t1.plant, t1.business_unit, t1.code, CASE WHEN sum(t1.forecast) <= 0::numeric OR sum(t1.sales) <= 0::numeric THEN 0.0 WHEN (abs(sum(t1.forecast) - sum(t1.sales)) / NULLIF(sum(t1.forecast), 0::numeric)) > 1::numeric THEN 0.0 ELSE abs(sum(t1.forecast) - sum(t1.sales)) / NULLIF(sum(t1.forecast), 0::numeric) END::numeric(18,10) AS valid_error_value FROM data_collection_forecast_accuracy t1 GROUP BY t1.year, t1.month, t1.week, t1.plant, t1.business_unit, t1.code ), weekly_accuracy AS ( SELECT t2.year, t2.month, t2.week, 1.0 - COALESCE(sum(t2.valid_error_value) FILTER (WHERE t2.valid_error_value > 0::numeric) / NULLIF(count(t2.valid_error_value) FILTER (WHERE t2.valid_error_value > 0::numeric), 0)::numeric, 0::numeric) AS overall_accuracy, 1.0 - COALESCE(sum(t2.valid_error_value) FILTER (WHERE t2.business_unit::text = 'fish'::text AND t2.valid_error_value > 0::numeric) / NULLIF(count(t2.valid_error_value) FILTER (WHERE t2.business_unit::text = 'fish'::text AND t2.valid_error_value > 0::numeric), 0)::numeric, 0::numeric) AS fish_accuracy, 1.0 - COALESCE(sum(t2.valid_error_value) FILTER (WHERE t2.business_unit::text = 'shrimp'::text AND t2.valid_error_value > 0::numeric) / NULLIF(count(t2.valid_error_value) FILTER (WHERE t2.business_unit::text = 'shrimp'::text AND t2.valid_error_value > 0::numeric), 0)::numeric, 0::numeric) AS shrimp_accuracy FROM code_weekly_metrics t2 GROUP BY t2.year, t2.month, t2.week ), plant_weekly_accuracy AS ( SELECT t3.year, t3.month, t3.week, t3.plant, 1.0 - COALESCE(sum(t3.valid_error_value) / NULLIF(count( CASE WHEN t3.valid_error_value > 0::numeric THEN 1 ELSE NULL::integer END), 0)::numeric, '-1.0'::numeric) AS plant_accuracy FROM code_weekly_metrics t3 GROUP BY t3.year, t3.month, t3.week, t3.plant ), best_worst_plants AS ( SELECT DISTINCT t4.year, t4.month, t4.week, first_value(t4.plant) OVER (PARTITION BY t4.year, t4.month, t4.week ORDER BY t4.plant_accuracy DESC, t4.plant RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS best_performing, first_value(t4.plant) OVER (PARTITION BY t4.year, t4.month, t4.week ORDER BY t4.plant_accuracy, t4.plant RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS worst_performing FROM plant_weekly_accuracy t4 WHERE t4.plant_accuracy >= 0::numeric ) SELECT m.year, m.month, m.week, CASE WHEN round(m.overall_accuracy, 4) = 0.0000 THEN NULL::numeric ELSE round(m.overall_accuracy, 4) END AS overall_accuracy, CASE WHEN round(m.fish_accuracy, 4) = 0.0000 THEN NULL::numeric ELSE round(m.fish_accuracy, 4) END AS fish_accuracy, CASE WHEN round(m.shrimp_accuracy, 4) = 0.0000 THEN NULL::numeric ELSE round(m.shrimp_accuracy, 4) END AS shrimp_accuracy, w.best_performing, w.worst_performing FROM weekly_accuracy m JOIN best_worst_plants w ON m.year = w.year AND m.month = w.month AND m.week = w.week ORDER BY m.year, m.month, m.week`);

export const summaryAccuracy = pgView("summary_accuracy", {	year: integer(),
    month: integer(),
    plant: varchar({ length: 50 }),
    businessUnit: varchar("business_unit", { length: 50 }),
